// -----------------------------------------------------------------------------
// Per-CTU MVP scheduler.  CU commands are queued by block size, neighbor reads
// are issued before candidate generation, and each active reference index is
// visited as required.  AMVP_OR_MRG selects the small sequencing differences
// between AMVP and Merge operation.
// -----------------------------------------------------------------------------
...
// The control state is one-hot.  There is one command FIFO and one AMVP
// reference counter per supported block size (index 0/1/2 = 8/16/32).
...
// Queue packet layout: cux[2:0], cuy[5:3], B availability[8:6],
// A availability[10:9], ZMV[11], skip[12], terminate[13].
// AVC suppresses an out-of-picture B2 position before the packet is queued.
...
generate
    if(AMVP_OR_MRG==1) begin : gen_amvp_ccu_cmd_push_pop_blk
        ...
        // AMVP revisits the same queued CU once per active reference index and
        // pops it only after the last reference has completed.
        ...
        // Merge is reference-index independent: one candidate transaction is
        // sufficient, so the command pops immediately at cand_blk_done.
...
// blk_sz_lat is a one-cycle entry pulse used by the neighbor manager to switch
// its logical A/B view to the block size selected by the scheduler.
...
if(AMVP_OR_MRG) begin : fsm_amvp_ctrl
    ...
    // For each CU: fetch neighbors once, process available sizes from small to
    // large, then loop back at BLK_DONE until every size has visited all refs.
    ...
    // Merge uses the same size traversal but has no per-reference loop.
    always@(*) begin : fsm_mvp_ctrl
        fsm_mvp_ns = 0;
        case(1) //synopsys parallel_case
            fsm_mvp_cs[AMVP_IDLE]: begin
...
generate
    if(AMVP_OR_MRG==1) begin : amvp_ref_idx_blk
        ...
        // A size not selected for the current CU is initialized to num_ref
        // (the done sentinel).  Selected sizes start at ref 0 and advance only
        // when their candidate transaction completes.
