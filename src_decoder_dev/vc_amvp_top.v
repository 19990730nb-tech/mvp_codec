...
// exchanges candidates with FME/CCU, and reconstructs AVC decoder motion as
// selected MVP + parsed MVD when codec_mode is asserted.
//
...
// [DECODER] codec_mode=0: {blk_sz[1:0], ref_idx[1:0], actual_mv_yx[31:0]}
//           codec_mode=1: {blk_sz[1:0], ref_idx[1:0], parsed_mvd_yx[31:0]}
...
wire signed [16:0]          dec_mv_sum_x;
wire signed [16:0]          dec_mv_sum_y;
...
always@(*) begin : amvp_itf_blk
	...
	for(i=0 ; i<MAX_BLK_SZ ; i++) begin
        ...
        // [DECODER] In decoder mode this sideband carries {ref_idx, parsed_mvd}.
        //           Reconstructed final MV is emitted later through irpu_amvp_rd.
...
// [DECODER] AVC mode always uses cand0 (median MVP). The 17-bit intermediate
//           makes the signed addition explicit; the stored MV remains the
//           original 16-bit two's-complement representation.
assign  selected_mvp_x = sel_cand_0[15: 0];
assign  selected_mvp_y = sel_cand_0[31:16];
assign  dec_mv_sum_x   = $signed({selected_mvp_x[15], selected_mvp_x}) +
                         $signed({mv_data_x[15], mv_data_x});
assign  dec_mv_sum_y   = $signed({selected_mvp_y[15], selected_mvp_y}) +
                         $signed({mv_data_y[15], mv_data_y});

// [DECODER] Encoder input is actual MV; decoder input is parsed MVD.
//           ZMV is an encoder-side override. Non-skip AVC decoder blocks
//           always reconstruct MV from MVP + parsed MVD.
assign	mvx = avc_dec_en ? dec_mv_sum_x[15:0] :
             (zmv ? 16'sd0 : mv_data_x);
assign	mvy = avc_dec_en ? dec_mv_sum_y[15:0] :
             (zmv ? 16'sd0 : mv_data_y);
...
// [DECODER] Cost-based MVP selection is encoder-only. AVC decoder uses cand0.
assign	cand_sel = ~codec_mode & ~reg_avc_mode &
                  (mvdcost_cand0_sum > mvdcost_cand1_sum);
// [DECODER] mvx/mvy are reconstructed final MV when avc_dec_en=1.
assign	irpu_amvp_wd[31: 0] = {mvy, mvx};
// [DECODER] Preserve parsed MVD bit-exactly instead of recomputing it from
//           the truncated 16-bit reconstructed MV.
assign	irpu_amvp_wd[47:32] = avc_dec_en ? mv_data_x :
                              (cand_sel ? mvd_cand1[0] : mvd_cand0[0]);
assign	irpu_amvp_wd[63:48] = avc_dec_en ? mv_data_y :
                              (cand_sel ? mvd_cand1[1] : mvd_cand0[1]);
