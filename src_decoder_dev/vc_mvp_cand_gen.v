// -----------------------------------------------------------------------------
// Shared AMVP/Merge candidate-generation engine.
// Candidate priority, duplicate removal, temporal scaling and the AVC median
// path are selected by AMVP_OR_MRG and the slice configuration inputs.
// cand_rdy bits may assert in different cycles; cand_blk_done ends the current
...
// Candidate-source abbreviations used throughout the selector/FSM:
// U/S = unscaled/scaled, A/B = spatial side, C = colocated, Z0 = zero MV.
// These values are bit positions in the one-hot candidate-selection vectors.
localparam	UA = 0,
			SA = 1,
			UB = 2,
			SB = 3;
...
// Merge source order follows the normative spatial priority and then appends
// temporal, scaled-temporal and zero candidates.  MED is the AVC median path.
localparam  A1 = 0,
			B1 = 1,
			B0 = 2,
			A0 = 3,
...
// neighbor available
// cu_cmd_out packet: block-size one-hot [16:14], terminate/skip/ZMV [13:11],
// A availability [10:9], B availability [8:6], CU y/x [5:3]/[2:0].
...
if(AMVP_OR_MRG) begin : amvp_output_sel
	// The generated 43-bit record is shared by AMVP and Merge:
	// {long_term[42], poc_diff[41:34], ref_idx[33:32], mvy[31:16], mvx[15:0]}.
	// cand0 and cand1 may be finalized in different cycles when the single
	// scaler is reused, so cand_rdy must be treated as independent valid bits.
	// unscale
	...
	// AVC derives X and Y medians independently from A, B and C.  Missing
	// neighbors are zero-masked; if exactly one source exists it is forwarded.
...
if(AMVP_OR_MRG == 0) begin : merge_cand_prior
    ...
    // Remove duplicate spatial candidates before filling the two output slots.
    // The FSM only waits when a selected temporal candidate requires scaling;
    // all-spatial and zero-fill combinations complete without an extra cycle.
    ...
    // AMVP tries two distinct spatial candidates first.  If one slot still
    // needs a scaled candidate, the WAIT_SCALE_* states serialize A, B and C
    // through the single vc_mvp_scale instance and suppress duplicates.
	...
	// cand_num marks that one output slot is already occupied while the scaler
	// works on the other slot.  inc/rst are derived from the same decisions as
	// the FSM so a returned scaled MV is written to the correct candidate.
	always@(*) begin
		cand_num_inc = 0;
		cand_num_rst = 0;
		case(1)
	...
	// Start the shared scaler only on a transition that actually selects a
	// scaled A/B/C source; WAIT states may immediately launch the next source.
	always@(*) begin
		scale_start = 0;
		case(1)
			fsm_cand_cs[CAND_IDLE]: begin
