// -----------------------------------------------------------------------------
// Combinational candidate-priority selector for vc_mvp_cand_gen.
// The one-hot cand_a/cand_b/cand_c outputs identify the preferred spatial or
// colocated source after availability, reference-picture and scaling checks.
// -----------------------------------------------------------------------------
...
generate
    if(AMVP_OR_MRG) begin : amvp_cand_con
        // An unscaled AMVP candidate must refer to the same picture; matching
        // long-term references are equivalent without comparing their POCs.
        // A scaled candidate only requires matching long/short-term class.
        ...
        if(MVP_SCALE_EN) begin: amvp_scale_en
            // Prefer an available A-side scaled candidate.  B-side scaling is
            // considered only when neither A position is available.
...
endgenerate
    ...
    // Temporal candidates are usable only for inter-coded colocated blocks.
    // Short-term candidates also compare POC distance; a differing distance
    // is routed to the scaler when scaling support is enabled.
...
generate
    if(AMVP_OR_MRG) begin : amvp_cand_one_hot
        // AMVP priority within each side is unscaled before scaled:
        // A0, A1, scaled-A0, scaled-A1 and B0, B1, B2, scaled-B0..B2.
        // Each group is encoded one-hot so downstream muxes use case(1).
        ...
        // Merge keeps all available spatial positions; duplicate filtering and
        // the normative A1/B1/B0/A0/B2 order are handled by cand_gen.
...
endgenerate
        ...
        // Temporal priority: unscaled C0, scaled C0, unscaled C1, scaled C1.
