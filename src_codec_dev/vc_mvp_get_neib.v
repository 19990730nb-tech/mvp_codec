// -----------------------------------------------------------------------------
// Neighbor-data manager shared by the AMVP and Merge schedulers.
// It arbitrates queued CU commands, requests spatial/colocated/reference-list
// memories, converts returned payloads into candidate-generator formats, and
// maintains within-CTU A/B neighbor history after CU updates.
// -----------------------------------------------------------------------------
...
// mvp_neib_*_reg are staging caches filled by external memory responses.
// a_0_reg/b_0_reg/buf_reg hold motion from CUs already processed in this CTU;
// later small blocks must use these newer values instead of stale memory data.
reg		[ 1:0]				fsm_neib_cs;
...
// Each block size touches a different number of packed memory words.  Odd 8x8
// rows/columns can reuse the within-CTU caches and therefore issue no A/B read.
...
// For cache-served 8x8 neighbors, the matching return may arrive before the
// other memory classes drain.  Sticky *_start_* flags remember that completion.
...
always@(*) begin : get_neib_a_b_blk
    // Response metadata is one-hot.  Replace only the addressed pair and keep
    // all other staging entries, allowing A/B reads to return independently.
    integer i;
    for(i=0 ; i<4 ; i++) begin
...
function [2:0][33:0] get_rd_neib_b;
    ...
    input             avc_mode;
begin
    // Convert the packed eight-entry B staging window into logical B0/B1/B2.
    // Block size and CU position determine which samples border the current CU.
...
function [1:0][33:0] get_rd_neib_a;
    ...
    input             avc_mode;
begin
    // Vertical counterpart of get_rd_neib_b: select logical A0/A1 from the
    // six-entry A staging window.
    if(blk_en[2]) begin
...
function [1:0][33:0] get_neib_a;
    ...
    input  [1:0]      cuy;
begin
    // An 8x8 CU in the right half of a 16x16 region takes A1 from the already
    // decoded block on its left; larger/left-edge blocks keep the memory value.
...
function [2:0][33:0] get_neib_b;
    ...
    integer i;
begin
    // Overlay within-CTU history on the external B window.  The special cases
    // cover raster-order boundaries where B2 lies in a previously decoded CU
    // rather than in the memory word selected for the current position.
...
// Capture returned A/B/colocated payloads into their metadata-selected staging
// slots.  These caches form the stable input snapshot used by both schedulers.
