...
// from a single sampled register interface.
...
// Merge and AMVP own separate command/candidate pipelines but share the
// neighbor manager below.  Their block-size latch pulses select the proper
// logical view of the common A/B/colocated staging caches.
ve_mrg_top
#(
...
// Centralize all external neighbor/reference memory requests so AMVP and Merge
// observe the same within-CTU updates and packed-memory address mapping.
vc_mvp_get_neib
#(
    .VC_CTU_X_NB	(VC_CTU_X_NB),
	.VC_CTU_Y_NB	(VC_CTU_Y_NB),
